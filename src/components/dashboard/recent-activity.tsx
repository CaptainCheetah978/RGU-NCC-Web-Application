...
export function RecentActivity() {
    ...
    return (
        <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-slate-700/60">
            ...
            <div className="relative pl-4 border-l-2 border-gray-100 dark:border-slate-700 space-y-6">
                {displayActivities.map((item, i) => (
                    <div key={i} className="relative">
                        ...
                        {item.time && <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">{item.time}</p>}
                    </div>
                ))}
            </div>
        </div>
